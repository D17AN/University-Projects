...
Repository::Repository(const std::string& sourceFile) : fileName{sourceFile}{
...
void Repository::saveToFile(){
	if (this->fileName == ""){
		throw RepositoryException("The repository has not been initialized with a file!");
	}
	...
	std::for_each(this->vector->begin(), this->vector->end(), [&](TrenchCoat& tc){
		if (&this->vector->back() != &tc){
			file << tc << "\n";
		}
		else{
			file << tc;
