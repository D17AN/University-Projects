...
TrenchCoat::TrenchCoat() : size{""}, colour{""}, price{0}, quantity{0}, photographLink{""}{}


TrenchCoat::TrenchCoat(const std::string& size, const std::string& colour, const int& price, const int& quantity, const std::string& photographLink){
	this->size = size;
	this->colour = colour;
	this->price = price;
...
bool TrenchCoat::operator==(const TrenchCoat& tc){
	return this->size == tc.size && this->colour == tc.colour && this->photographLink == tc.photographLink;
}
...
{
	return this->size + "|" + this->colour + "|" + std::to_string(this->price) + "|" + std::to_string(this->quantity) + "|" + this->photographLink;
}


std::ostream& operator<<(std::ostream& os, const TrenchCoat& tc){
	os << tc.size << "|" << tc.colour << "|" << tc.price << "|" << tc.quantity << "|" << tc.photographLink;
	return os;
}
...
std::istream& TrenchCoat::read(std::istream& is){
	...
	if (TrenchCoatValidator::isColourValid(color) == false){
...
bool TrenchCoatValidator::isColourValid(const std::string& colour){
	if (colour == "")
		return false;
	return true;
}
