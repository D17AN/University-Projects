...
void RepositoryTests::testCopyConstructor(){
	...
	repo1.add(tc1);
	assert(repo1.get().size() == 1);
	
	Repository repo2{repo1};
	assert(repo2.get().size() == 1);
...
void ControllerUserTests::testAddToBasket(){
	...
	repo.add(tc1);
	repo.add(tc2);
	assert(cu.getRepo("").size() == 2);
	cu.addToBasket(tc1);
	cu.addToBasket(tc1);
	assert(cu.getBasket().size() == 1);
	assert(cu.getBasket()[0].getQuantity() == 2);
	assert(basket.get().size() == 1);
	cu.addToBasket(tc2);
	assert(cu.getBasket().size() == 2);
}
