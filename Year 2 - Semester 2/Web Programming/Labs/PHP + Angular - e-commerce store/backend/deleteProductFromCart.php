...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $deleteSql = "DELETE FROM CART WHERE PRODUCT_ID = $productId LIMIT 1";

        if (mysqli_query($con, $deleteSql)) {
            $response = array("success" => true, "message" => "Product removed from the cart successfully.");
        } else {
            $response = array("success" => false, "message" => "Error: " . mysqli_error($con));
        }
    } else {
        $response = array("success" => false, "message" => "Product not found in the cart.");
    }
