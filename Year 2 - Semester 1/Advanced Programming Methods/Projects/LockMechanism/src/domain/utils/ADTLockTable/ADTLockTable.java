...
public class ADTLockTable implements ADTLockTableInterface {
    ...
    @Override
    public void put(int key, int value) throws ToyLanguageException {
        synchronized (this) {
            if (!lockTable.containsKey(key)) {
                lockTable.put(key, value);
            } else {
                throw new ToyLanguageException(String.format("Lock table already contains the key %d!", key));
            }
        }
    }
    ...
    public int get(int position) throws ToyLanguageException {
        synchronized (this) {
            if (!lockTable.containsKey(position))
                throw new ToyLanguageException(String.format("%d is not present in the table!", position));
            return lockTable.get(position);
        }
    }

    @Override
    public void update(int position, int value) throws ToyLanguageException {
        synchronized (this) {
            if (lockTable.containsKey(position)) {
